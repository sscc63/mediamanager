...
// 「名称」是 yun123pan 的写法，漏了它整个频道会搜不到 TMDB。
...
func NewMessageDB(dbPath string) *MessageDB {
	...
	// 老库 usedate 为 NULL 必须回填，否则会回退到 recognized_at 而被当成未过期。
	m.backfillUseDate()
...
func (m *MessageDB) backfillUseDate() {
	...
	rows, err := m.db.Query("SELECT msg_id, date FROM messages WHERE COALESCE(usedate,'') = '' AND date <> ''")
	if err != nil {
		log.Printf("[监控] usedate 回填查询失败: %v", err)
		return
	}
	ids, vals, bad := []int64{}, []string{}, 0
	for rows.Next() {
		var id int64
		var d string
...
func (m *MessageDB) ListRecentWithTitle(hours int, limit int) []ChannelRecent {
	...
	// target_url/transfer_time/message_url 必须 COALESCE：老库里为 NULL 时 Scan 进 string
	// 会报错，该行被静默跳过（err 为 nil、无日志）。
